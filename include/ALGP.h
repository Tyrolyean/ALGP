...
namespace ALGP {
	...
	public:
		...
		/*
		 * Network stuff.
		 * As this is used in the client as well as the server it is configured globally.
		 * The ability to add a different port for each address was considered, but
		 * deemed unnescessary at this point in time. It may be added in the future
		 * though so that's still a thing…
		 */

		bool set_local_addresses(std::vector<std::string> laddr);
		std::vector<std::string> get_local_addresses();
		void force_set_local_addresses(std::vector<std::string> laddr);
	...
	private:
		...
		std::vector<std::string> local_addr;
