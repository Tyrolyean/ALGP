...
namespace ALGP {
	...
	bool ALGP::add_output(std::ostream* output, std::vector<bool> output_allow, bool color_allow) {
		...
		if (output != NULL && output_allow.size() == (output_type::END_CALL + 1)) {
			...
			if (success == true) {
				...
				// Please note that colors are only supported in linux
				// at this point in time.
				std::pair < std::ostream*, std::pair < std::vector<bool>, bool>> 
					fin(output, tmp);
	...
	bool ALGP::set_local_addresses(std::vector<std::string> laddr) {

		if (get_connection_state() != 0) {
			Output::println(output_type::WARNING, 
					"Tried to override the connection lock! ADDRESS",this);
			return false;
		} else {
	...
	std::vector<std::string> ALGP::get_local_addresses() {
		return this->local_addr;
	}

	unsigned short int ALGP::get_local_port(){
		return this->local_port;
	}

	bool ALGP::set_local_port(unsigned short int lport){
		if(get_connection_state() != 0){
			Output::println(output_type::WARNING,
					"Tried to override the connection lock! PORT", this);
			return false;
		}else{
		...
		return false; // ALl BEHAIL ANSI
	}
    
	void ALGP::force_set_local_addresses(std::vector<std::string> laddr) {

		unsigned short int temp = get_connection_state();
		this->set_connection_state(0);
		if(!set_local_addresses(laddr)){
